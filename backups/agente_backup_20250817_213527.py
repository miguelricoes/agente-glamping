...
try:
    ...
    log_startup(logger, "Flask-SQLAlchemy", "SUCCESS", "Importación exitosa")
except ImportError:
...
from utils.logger import get_logger, log_startup, log_error, log_database_operation

# Inicializar logger para este módulo
logger = get_logger(__name__)


logger.info(f"Puerto asignado: {os.environ.get('PORT') or 'default'}", extra={"port": os.environ.get('PORT'), "phase": "startup"})
...
try:
    ...
    log_startup(logger, "Pinecone", "SUCCESS", "ServerlessSpec disponible")
except ImportError:
    try:
        # Import básico sin ServerlessSpec
        from pinecone import Pinecone
        PINECONE_SERVERLESS_AVAILABLE = False
        log_startup(logger, "Pinecone", "SUCCESS", "Sin ServerlessSpec")
    except ImportError:
        #  Import de legacy
        try:
            ...
            PINECONE_SERVERLESS_AVAILABLE = False
            log_startup(logger, "Pinecone", "SUCCESS", "API legacy")
        except ImportError:
...
def validate_environment_variables():
    ...
    if missing_critical:
        ...
        logger.critical("Variables críticas faltantes", extra={"missing_vars": missing_critical, "phase": "startup"})
    ...
    if missing_optional:
        ...
        logger.warning("Variables opcionales faltantes", extra={"missing_vars": missing_optional, "phase": "startup"})
    
    log_startup(logger, "Environment Variables", "SUCCESS", "Variables críticas configuradas")
    return True
...
if PINECONE_API_KEY:
    try:
        if hasattr(Pinecone, '__call__'):
            ...
            log_startup(logger, "Pinecone Client", "SUCCESS", "v3.0+ inicializado")
        else:
            ...
            log_startup(logger, "Pinecone Client", "SUCCESS", "Legacy inicializado")
    except Exception as e:
        warning_msg = f"WARNING: Error al inicializar Pinecone: {e}\n[TIP] Funcionalidad Pinecone deshabilitada"
        logger.warning(f"Error inicializar Pinecone: {e}", extra={"component": "Pinecone", "phase": "startup"})
        pc = None
else:
    logger.warning("PINECONE_API_KEY no configurada", extra={"component": "Pinecone", "phase": "startup", "impact": "Funcionalidad deshabilitada"})
...
if pc:
    try:
        ...
        try:
            ...
            logger.warning(f"No se pudo obtener lista de índices: {list_error}", extra={"component": "Pinecone", "phase": "startup"})
        ...
        if indexes and PINECONE_INDEX_NAME not in indexes:
            ...
            logger.warning(f"Índice '{PINECONE_INDEX_NAME}' no existe", extra={"component": "Pinecone", "available_indexes": indexes, "phase": "startup"})
            pinecone_index = None
        else:
            # Conectar al índice
            try:
                ...
                log_startup(logger, "Pinecone Index", "SUCCESS", f"Conectado a {PINECONE_INDEX_NAME}")
    ...
    logger.info("Pinecone no inicializado - Usando solo FAISS local", extra={"phase": "startup"})
...
if DATABASE_PRIVATE_URL:
    ...
    logger.warning("Usando DATABASE_PUBLIC_URL (puede generar costos de egress)", extra={"phase": "startup"})
...
if database_url:
    ...
    try:
        ...
        with app.app_context():
            try:
                ...
                log_startup(logger, "Conexion a base de datos verificada exitosamente", "SUCCESS", "")
            except Exception as conn_error:
                logger.error(f"Error de conexion a BD: {conn_error}", extra={"phase": "startup"})
    ...
    logger.warning("Ninguna DATABASE_URL configurada - Funcionalidad de base de datos deshabilitada", extra={"phase": "startup"})
...
def calcular_precio_reserva(domo, cantidad_huespedes, fecha_entrada, fecha_salida, servicios_adicionales=None):
    ...
    try:
        ...
        logger.info(f"Error calculando precio: {e}", extra={"phase": "info"})
        return {
...
def initialize_database():
    ...
    if not db:
        logger.info("Base de datos no configurada - Saltando inicialización", extra={"phase": "startup"})
        return False
    ...
    try:
        ...
        with app.app_context():
            ...
            db.session.commit()
        log_startup(logger, "Base de datos inicializada y tablas creadas correctamente", "SUCCESS", "")
        return True
    except Exception as e:
...
try:
    ...
    logger.info(f"Directorio de memoria creado: {MEMORY_DIR}", extra={"phase": "startup"})
except Exception as e:
...
try:
    ...
    log_startup(logger, "Cliente Twilio inicializado correctamente", "SUCCESS", "")
except Exception as e:
...
try:
    ...
    try:
        ...
        log_startup(logger, "LLM OpenAI inicializado con import tradicional", "SUCCESS", "")
        ...
        try:
            ...
            log_startup(logger, "LLM OpenAI inicializado con nuevo import langchain_openai", "SUCCESS", "")
        except ImportError:
            try:
                # Import legacy
                from langchain.llms.openai import OpenAI as OpenAI_Legacy
                llm = OpenAI_Legacy(temperature=0)
                log_startup(logger, "LLM OpenAI inicializado con import legacy", "SUCCESS", "")
            except ImportError:
                #  ChatOpenAI como fallback
                try:
                    from langchain.chat_models import ChatOpenAI
                    llm = ChatOpenAI(temperature=0, model="gpt-3.5-turbo")
                    log_startup(logger, "LLM inicializado con ChatOpenAI como fallback", "SUCCESS", "")
                except ImportError:
                    try:
                        ...
                        log_startup(logger, "LLM inicializado con ChatOpenAI nuevo", "SUCCESS", "")
                    except ImportError:
...
def call_chain_safe(chain_name: str, query: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error en cadena {chain_name}: {e}", extra={"phase": "startup"})
...
def consultar_disponibilidades_glamping(consulta_usuario: str) -> str:
    ...
    """
    try:
        logger.info(f"[AGENTE] Consultando disponibilidades: {consulta_usuario}", extra={"phase": "info"})
        ...
        logger.info(f"📊 [AGENTE] Parámetros extraídos: {parametros}", extra={"phase": "info"})
        ...
        logger.info(f"✅ [AGENTE] Respuesta: {respuesta[:100]}...", extra={"phase": "info"})
        return respuesta

    except Exception as e:
        logger.info(f"ERROR [AGENTE] Error en consulta: {str(e)}", extra={"phase": "info"})
        import traceback
        traceback.print_exc()
...
def obtener_disponibilidades_calendario(fecha_inicio, fecha_fin, domo_especifico=None, personas=None):
    ...
    """
    try:
        logger.info(f"🗄️ [BD] Consultando reservas desde {fecha_inicio} hasta {fecha_fin}", extra={"phase": "info"})
        ...
        logger.info(f"📊 [BD] Encontradas {len(reservas_activas)} reservas activas", extra={"phase": "info"})
        ...
        logger.info(f"✅ [BD] Consulta exitosa: {len(domos_disponibles_total)} domos disponibles", extra={"phase": "info"})
        return resultado

    except Exception as e:
        logger.info(f"ERROR [BD] Error en consulta: {str(e)}", extra={"phase": "info"})
        return {
            'success': False,
            'error': str(e),
...
def generar_respuesta_disponibilidades(datos, parametros, consulta_original):
    ...
    try:
        ...
        logger.info(f"ERROR Error generando respuesta: {e}", extra={"phase": "info"})
...
def save_user_memory(user_id: str, memory: ConversationBufferMemory):
    ...
    try:
        ...
        if not memory or not hasattr(memory, 'chat_memory') or not hasattr(memory.chat_memory, 'messages'):
            logger.warning(f" Memoria inválida para usuario {user_id}, saltando guardado", extra={"phase": "startup"})
            return False
        ...
        try:
            ...
            logger.error(f"Error al serializar mensajes para usuario {user_id}: {e}", extra={"phase": "startup"})
            return False
        ...
        if os.path.exists(memory_path):
            try:
                ...
                logger.warning(f" Archivo de memoria actual corrupto para usuario {user_id}: {e}", extra={"phase": "startup"})
        ...
        logger.info(f"Memoria guardada correctamente para usuario: {user_id}", extra={"phase": "startup"})
        return True
        
    except Exception as e:
        logger.error(f"Error inesperado al guardar memoria para usuario {user_id}: {e}", extra={"phase": "startup"})
...
def _create_fresh_memory(user_id: str) -> ConversationBufferMemory:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Memoria creada con API nueva para usuario: {user_id}", extra={"phase": "startup"})
        except (ImportError, AttributeError):
            try:
            
                memory.chat_memory.add_user_message(system_message)
                memory.chat_memory.add_ai_message(assistant_response)
                logger.info(f"Memoria creada con API legacy para usuario: {user_id}", extra={"phase": "startup"})
            except AttributeError:
                
                logger.warning(f" Creando memoria básica sin mensajes iniciales para usuario: {user_id}", extra={"phase": "startup"})
                
        return memory
        
    except Exception as e:
        logger.error(f"Error creando memoria para usuario {user_id}: {e}", extra={"phase": "startup"})
...
def _try_load_memory_from_file(file_path: str, user_id: str) -> tuple[bool, ConversationBufferMemory]:
    ...
    try:
        ...
        if os.path.getsize(file_path) == 0:
            logger.warning(f" Archivo de memoria vacío para usuario {user_id}: {file_path}", extra={"phase": "startup"})
            return False, None
        ...
        if not isinstance(serialized_messages, list):
            logger.warning(f" Formato de memoria inválido para usuario {user_id}: esperaba lista", extra={"phase": "startup"})
            return False, None
        ...
        try:
            ...
            try:
                ...
                logger.warning(f" No se pudieron cargar mensajes históricos para usuario {user_id}", extra={"phase": "startup"})
                return True, memory  # Retornar memoria vacía pero válida
        except Exception as e:
            logger.warning(f" Error al convertir mensajes para usuario {user_id}: {e}", extra={"phase": "startup"})
            return False, None
        
        return True, memory
        
    except json.JSONDecodeError as e:
        logger.warning(f" JSON corrupto para usuario {user_id} en {file_path}: {e}", extra={"phase": "startup"})
        return False, None
    except Exception as e:
        logger.warning(f" Error al leer archivo de memoria para usuario {user_id}: {e}", extra={"phase": "startup"})
        return False, None
...
def load_user_memory(user_id: str) -> ConversationBufferMemory:
    ...
    if success and memory:
        logger.info(f"Memoria cargada desde archivo principal para usuario: {user_id}", extra={"phase": "startup"})
        return memory
    
    # Intento 2: Cargar desde backup si el archivo principal falló
    if os.path.exists(backup_path):
        logger.info(f"🔄 Intentando recuperar desde backup para usuario: {user_id}", extra={"phase": "info"})
        success, memory = _try_load_memory_from_file(backup_path, user_id)
        if success and memory:
            logger.info(f"Memoria recuperada desde backup para usuario: {user_id}", extra={"phase": "startup"})
            # Intentar restaurar el archivo principal desde backup
            try:
                import shutil
                shutil.copy2(backup_path, memory_path)
                logger.info(f"🔧 Archivo principal restaurado desde backup para usuario: {user_id}", extra={"phase": "info"})
            except Exception as e:
                logger.warning(f" No se pudo restaurar archivo principal para usuario {user_id}: {e}", extra={"phase": "startup"})
            return memory
    
    #   crear memoria nueva si falla algo
    logger.info(f"Creando memoria nueva para usuario: {user_id}", extra={"phase": "info"})
    memory = _create_fresh_memory(user_id)
    
    # Guardar la memoria nueva
    if save_user_memory(user_id, memory):
        logger.info(f"Memoria nueva guardada para usuario: {user_id}", extra={"phase": "startup"})
    else:
        logger.warning(f" No se pudo guardar memoria nueva para usuario: {user_id}", extra={"phase": "startup"})
            ...
            if filename.endswith('.json') and not filename.endswith('_backup.json'):
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(f" No se pudo mover archivo corrupto {filename}: {move_error}", extra={"phase": "startup"})
        
        if corrupted_files:
            logger.info(f"🧹 Limpieza de memoria: {cleaned_count} archivos corruptos movidos", extra={"phase": "info"})
            for filename, error in corrupted_files[:5]:  # Solo mostrar primeros 5
                logger.info(f"   - {filename}: {error}", extra={"phase": "info"})
        else:
            log_startup(logger, "Todos los archivos de memoria están en buen estado", "SUCCESS", "")
            
    except Exception as e:
        logger.error(f"Error durante limpieza de archivos de memoria: {e}", extra={"phase": "startup"})
...
def save_reservation_to_pinecone(user_phone_number, reservation_data):
    try:
        if not reservation_data or not user_phone_number:
            logger.warning(f" Datos de reserva incompletos para Pinecone", extra={"phase": "startup"})
            return False
        ...
        logger.info(f"Reserva guardada en Pinecone para {user_phone_number}", extra={"phase": "startup"})
        return True
        
    except Exception as e:
        logger.error(f"Error al guardar en Pinecone para {user_phone_number}: {e}", extra={"phase": "startup"})
        return False
...
def parse_reservation_details(user_input):
    ...
    if not success:
        logger.error(f"Error al llamar al LLM para parsing: {error_msg}", extra={"phase": "startup"})
        return None
    ...
    if not json_success:
        logger.error(f"Error al parsear JSON: {json_error}", extra={"phase": "startup"})
        return None
    ...
    if missing_fields:
        logger.warning(f" JSON incompleto, faltan campos: {missing_fields}", extra={"phase": "startup"})
    ...
    log_startup(logger, "Datos de reserva parseados exitosamente", "SUCCESS", "")
    return parsed_json
    ...
    try:
        ...
        if not names_valid:
            ...
            if numero_acompanantes and str(numero_acompanantes).isdigit():
                ...
                logger.info(f"Número de acompañantes especificado: {numero_acompanantes}", extra={"phase": "startup"})
            else:
                processed_data["cantidad_huespedes"] = len(validated_names)
                logger.info(f"{names_msg}", extra={"phase": "startup"})
        ...
        if entrada_valid and salida_valid:
            ...
            if not range_valid:
                ...
                logger.info(f"{range_msg}", extra={"phase": "startup"})
...
def call_llm_with_retry(prompt: str, max_retries: int = 3, temperature: float = 0) -> tuple[bool, str, str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"LLM respondió exitosamente (intento {attempt + 1})", extra={"phase": "startup"})
            return True, response, "Éxito"
            
        except Exception as e:
            last_error = f"Error en intento {attempt + 1}: {str(e)}"
            logger.warning(f" {last_error}", extra={"phase": "startup"})
    ...
    try:
        ...
        for match in json_matches:
            try:
                ...
                log_startup(logger, "JSON extraído de respuesta del LLM", "SUCCESS", "")
        ...
        if match:
            try:
                ...
                log_startup(logger, "JSON anidado extraído de respuesta del LLM", "SUCCESS", "")
        ...
        try:
            ...
            log_startup(logger, "JSON parseado después de limpiar", "SUCCESS", "")
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                logger.info(f"Agente moderno inicializado correctamente (intento {attempt + 1})", extra={"phase": "startup"})
                ...
                logger.info(f"Agente tradicional inicializado correctamente (intento {attempt + 1})", extra={"phase": "startup"})
            ...
            logger.error(f"{last_error}", extra={"phase": "startup"})
...
def run_agent_safe(agent, user_input: str, max_retries: int = 2) -> tuple[bool, str, str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Agente ejecutado exitosamente (intento {attempt + 1})", extra={"phase": "startup"})
...
logger.info("Sistema inicializado - Iniciando rutas Flask", extra={"phase": "startup", "status": "ready"})
...
def handle_availability_request(message):
    ...
    try:
        ...
        logger.info(f"Error procesando consulta de disponibilidad: {e}", extra={"phase": "info"})
...
def is_glamping_related(message):
    ...
    try:
        ...
        logger.info(f"[FILTRO] Mensaje: '{message[:50]}...' -> Relacionado: {is_related}", extra={"phase": "info"})
        return is_related
        
    except Exception as e:
        logger.info(f"Error en filtro de temas: {e}", extra={"phase": "info"})
...
#     pass
    ...
    if is_menu_selection(incoming_msg) and user_state["current_flow"] == "none":
        try:
            ...
            logger.info(f"Error en manejo de menú: {e}", extra={"phase": "info"})
        ...
        if incoming_msg.lower() in ["si", "sí"]:
            try:
                ...
                try:
                    ...
                    db.session.commit()
                    logger.info(f"Reserva guardada en PostgreSQL - ID: {nueva_reserva.id}", extra={"phase": "startup"})
                    ...
                    db.session.rollback()
                    logger.error(f"Error al guardar en base de datos: {db_error}", extra={"phase": "startup"})
                    resp.message(
    ...
    try:
        ...
        if not init_success:
            logger.error(f"Error al inicializar agente: {init_error}", extra={"phase": "startup"})
            ...
            if run_success:
                ...
                logger.error(f"Error ejecutando agente: {run_error}", extra={"phase": "startup"})
        ...
        logger.error(f"Error inesperado en procesamiento conversacional: {e}", extra={"phase": "startup"})
    ...
    if agent_answer == "REQUEST_RESERVATION_DETAILS":
        ...
        logger.info(f"[{from_number}] Respuesta: '{agent_answer}'", extra={"phase": "info"})
    return str(resp)
...
# TEMPORARILY COMMENTED: Moved to routes/chat_routes.py
# Original chat function commented out to avoid conflicts
# The new modular version is loaded from routes/chat_routes.py
# @app.route("/chat", methods=["POST"])
# def chat():
#     Full function implementation moved to routes/chat_routes.py
#     pass
# Everything from here to the end of the chat function has been moved to routes/chat_routes.py
...
def get_reservas():
    ...
    try:
        ...
        logger.info(f"Error en endpoint GET /api/reservas: {str(e)}", extra={"phase": "info"})
        return jsonify({
            'success': False,
...
def get_reservas_stats():
    ...
    try:
        ...
        logger.info(f"ERROR en get_reservas_stats: {e}", extra={"phase": "info"})
...
def consultar_disponibilidades_interna(fecha_inicio=None, fecha_fin=None, domo=None, personas=None):
    ...
    try:
        ...
        logger.info(f"ERROR Error en consultar_disponibilidades_interna: {str(e)}", extra={"phase": "info"})
        return {
            'success': False,
            'error': str(e),
            'disponibilidades_por_fecha': {},
...
def get_disponibilidades():
    ...
    try:
        ...
        logger.info(f"ERROR Error en get_disponibilidades: {str(e)}", extra={"phase": "info"})
        return jsonify({
            'success': False,
            'error': str(e),
            'disponibilidades_por_fecha': {},
...
def agente_consultar_disponibilidades():
    ...
    try:
        ...
        logger.info(f"Error en /api/agente/disponibilidades: {str(e)}", extra={"phase": "info"})
        return jsonify({
...
def extraer_parametros_consulta(consulta):
    ...
    logger.info(f"[ANALISIS] Analizando consulta: '{consulta_lower}'", extra={"phase": "info"})
    ...
    for patron, tipo in patrones_fecha:
        ...
        if matches:
            try:
                match = matches[0]
                logger.info(f"📅 Fecha encontrada: {match}, tipo: {tipo}", extra={"phase": "info"})
                ...
                logger.info(f"✅ Fecha procesada: {parametros['fecha_inicio']}", extra={"phase": "info"})
                break

            except Exception as e:
                logger.info(f"ERROR Error procesando fecha {match}: {e}", extra={"phase": "info"})
                continue
    ...
    for domo, aliases in domos_nombres.items():
        for alias in aliases:
            if alias in consulta_lower:
                ...
                logger.info(f"🏠 Domo detectado: {domo}", extra={"phase": "info"})
                break
    ...
    for patron in patrones_personas:
        ...
        if match:
            ...
            logger.info(f"👥 Personas detectadas: {parametros['personas']}", extra={"phase": "info"})
            break
    ...
    logger.info(f"📊 Parámetros finales extraídos: {parametros}", extra={"phase": "info"})
    return parametros
...
def generar_respuesta_natural_disponibilidades(datos_disponibilidades, parametros_extraidos, consulta_original):
    ...
    try:
        ...
        logger.info(f"ERROR Error generando respuesta natural: {e}", extra={"phase": "info"})
        return "Tenemos domos disponibles. ¿Te gustaría hacer una reserva?"
...
def authenticate_user(email, password):
    ...
    try:
        ...
        logger.info(f"Error en authenticate_user: {e}", extra={"phase": "info"})
        return None
...
def login():
    ...
    try:
        ...
        logger.info(f"Error en login endpoint: {e}", extra={"phase": "info"})
...
try:
    ...
    log_startup(logger, "Módulo WhatsApp routes registrado correctamente", "SUCCESS", "")
except ImportError as e:
    logger.warning(f"No se pudo importar el módulo WhatsApp routes: {e}", extra={"phase": "startup"})
    logger.info("Usando implementación original en agente.py", extra={"phase": "startup"})
